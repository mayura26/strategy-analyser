...
export default function AnalysisPage() {
  ...
  const getFilteredChartData = () => {
    ...
    if (showSignificantDifferences && selectedRuns.length > 1) {
      filteredData = filteredData.filter(dataPoint => {
        const values = sortedRuns.map(runId => dataPoint[`run_${runId}`] || 0);
        
        // Check if any pair of runs has a difference greater than the threshold
        for (let i = 0; i < values.length; i++) {
          for (let j = i + 1; j < values.length; j++) {
            const difference = Math.abs(values[i] - values[j]);
          ...
          <CardContent className="p-3">
            <div className="flex items-center justify-between">
                  ...
                  {selectedRuns.length > 1 && (
                    <div className="flex items-center gap-2">
                      <label className="flex items-center gap-1 text-xs text-gray-400 hover:text-gray-300 cursor-pointer group">
                        <input
                          type="checkbox"
                          checked={showSignificantDifferences}
                          onChange={(e) => setShowSignificantDifferences(e.target.checked)}
                        ...
                        <div className="relative group">
                          <Info className="h-3 w-3 text-gray-500" />
                          <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-2 py-1 bg-gray-900 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10">
                            Show only days with significant PnL differences (threshold: ${significanceThreshold})
                          </div>
                        </div>
                      </label>
                    </div>
              ...
              <div className="flex items-center gap-2">
                ...
                {showSignificantDifferences && selectedRuns.length > 1 && (
              ...
              <div className="mt-4 pt-4 border-t border-gray-700 space-y-3">
                ...
                {selectedRuns.length > 1 && (
                  <>
                    ...
                    <div className="text-xs text-gray-500">
                      <strong>Significant Differences:</strong> Shows only days where the PnL difference between runs exceeds the threshold (set to $1 to hide identical days)
                    </div>
                    
                    {/* Significance Threshold Control */}
                    <div className="flex items-center gap-3 mt-3">
                      <label className="text-xs text-gray-400">Significance Threshold:</label>
                      <div className="flex items-center gap-2">
                        <input
                          type="number"
                          value={significanceThreshold}
                          onChange={(e) => setSignificanceThreshold(Math.max(0, parseInt(e.target.value) || 0))}
                    ...
                    {showOverlapOnly && getOverlappingDateRange(selectedRuns) && (
                      <div className="block text-blue-400 mt-1">
                        <div>
                          ...
                          {showSignificantDifferences && selectedRuns.length > 1 && ` (filtered to show only significant differences ≥$${significanceThreshold})`}
                          {!showSignificantDifferences && ` (${getOverlappingDateRange(selectedRuns)?.dates.length} overlapping days)`}
                        </div>
                        {selectedRuns.length > 0 && (() => {
